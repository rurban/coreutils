#!/bin/sh
# Test whether cp -g prints the sizes.
...
# test miscellaneous combinations of -f -g -n parameters
touch c d || framework_failure_
echo "'c' -> 'd'" > out_copy
echo "of" > out_progress
> out_empty

cp -vg c d 2>/dev/null > out1 || fail=1
compare out1 out_empty || fail=1

# ask for overwrite, answer no
#echo n | cp -vg  c d 2>/dev/null > out1 || fail=1
#compare out1 out_empty || fail=1
